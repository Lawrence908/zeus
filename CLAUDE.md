...
| MCP | `zeus/mcp/server.py` (FastMCP) | Memory tools: `zeus_query`, `zeus_profile`, `zeus_remember`, `zeus_ingest_trigger`, `zeus_memory_search`. Olympian read tools: `olympian_status_read`, `olympian_server_health`, `olympian_file_read`, `olympian_file_search`, `olympian_action_list`, `zeus_calendar_today`, `zeus_newsletter_latest`, `zeus_image_generate` (gated by `ZEUS_IMAGE_ENABLED`). Olympian write tools (gated by `ZEUS_MCP_ALLOW_WRITE`): `olympian_inbox_append`, `olympian_action_run`. The action runner additionally requires `ZEUS_ACTIONS_ENABLED=1`. Each tool is mirrored as a chat-path `ToolSpec` in `zeus/core/tools/` and exposed through both surfaces. |
