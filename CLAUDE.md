...
| MCP | `zeus/mcp/server.py` (FastMCP) | Tools: `zeus_query`, `zeus_profile`, `zeus_remember`, `zeus_ingest_trigger`, `zeus_memory_search`. Write tools gated by `ZEUS_MCP_ALLOW_WRITE`. |
...
- **Tool-first.** Agents call concrete tools rather than reasoning in chat. Current MCP tools live in `zeus/mcp/tools.py`: `zeus_query`, `zeus_profile`, `zeus_remember`, `zeus_ingest_trigger`, `zeus_memory_search`. Write tools are gated by `ZEUS_MCP_ALLOW_WRITE`. Kairos draws from a separate allowlist (`ZEUS_KAIROS_TOOL_ALLOWLIST`, default read-only).
