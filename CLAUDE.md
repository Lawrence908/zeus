...
- **Editable prompts:** The runtime chat system prompt lives in [`zeus/core/prompts/chat_system.md`](zeus/core/prompts/chat_system.md) and is rendered by `zeus/core/prompts/__init__.py` with `{{PLACEHOLDER}}` markers. Set `ZEUS_PROMPT_RELOAD=1` against a running `zeus-core` to re-read templates from disk on every call during iteration; defaults to process-cached in production. Edit the `.md` file to iterate on persona, grounding rules, or context-block instructions without touching Python.
...
- **No emdashes in generated text**: Never use emdashes (the long dash character) in assistant replies, user-facing responses, or text generated for Chris. Use commas, semicolons, colons, or restructure sentences instead. This rule applies to model output; internal repo docs (including this file) may still contain emdashes from earlier revisions and are not the authoritative style reference.
