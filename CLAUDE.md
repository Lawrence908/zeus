...
- **Structured memory:** `QueryEngine` (`zeus/core/query.py`) fetches up to 5 relevant chunks via `search_memories()` and profile facts via `get_profile_facts()` before each LLM call. Session context comes from `SessionManager.get_context_window()` which interleaves rolling summaries (triggered at 20 turns) with the last 10 raw turns. Durable storage is mem0 + Qdrant; ephemeral state stays in `app.state` or `SQLiteSessionStorage`. Do not grow raw transcript context — summarise.
