...
| News (Pheme) | `zeus/memory/news.py` (`NewsStore`) + `zeus/pheme/` | Qdrant collection `zeus_news` (dense, deterministic ids, retention `NEWS_RETENTION_DAYS`). Canary + CapitolScope ingest via `target: news`; staged local-Ollama pipeline (extract/cluster/thread/correlate/rank/synthesize); Kronos daily digest + KAIROS breaking observer; Telegram push + approval-gated Twitter. See `zeus/docs/pheme.md`. |
| Image gen (Pygmalion) | `zeus/core/comfyui.py` + `zeus/core/tools/image_generate.py` | ComfyUI on the two GPUs: Apollo RTX 5080 FLUX primary, always-on daedalus RTX 3080 SDXL `--lowvram` fallback (`services/comfyui/`). Failover in-process (health-check `/system_stats`), not a Caddy LB, because ComfyUI's `/prompt`→poll→`/view` exchange is stateful. Graphs built in Python; filenames/model per node env-overridable (`ZEUS_COMFYUI_*`). Gated by `ZEUS_IMAGE_ENABLED`; PNGs persist under `zeus_data`, served at `/images/<file>`. Chat tool `image_generate`, MCP `zeus_image_generate`. See `homelab-docs/network.md` §9a. |
...
- **pheme** news consolidation and analysis (NewsStore over `zeus_news`, staged local pipeline, daily digest + breaking alerts, Telegram push, gated Twitter)
- **orpheus** voice interface (STT + TTS + wake word)
- **phaos** voice-state visualization (WebSocket + Three.js orb)
- **pygmalion** image generation (ComfyUI over the 5080/3080; `image_generate` tool)
...
- **Chat-path tool-use.** Gated by `ZEUS_TOOLS_ENABLED` (code default `0`; enabled on the live deploy), `QueryEngine.query()` and `query_stream()` route through `run_tool_loop()` in `zeus/core/tools/loop.py`: the chat LLM can emit tool calls, results are fed back, loop continues until it stops calling tools or hits `ZEUS_TOOLS_MAX_CALLS_PER_QUERY` (default 5). Tools live in a process-local registry (`zeus/core/tools/registry.py`); `allowed_tool_specs()` filters them by `ZEUS_TOOLS_ALLOWLIST` (unset = all) so prod can run a read-only subset while dev runs the full pack; the model is only told about tools it may call. Per-provider wire-format adapters for Anthropic and Ollama are in `zeus/core/tools/adapters.py`. Every tool argument dict and every tool result text passes through Aegis under the `tool_arguments` policy. Reflection is skipped for tool-informed replies. Invocations are recorded (`zeus/core/tools/recorder.py`) and rolled up into `GET /admin/metrics` (`tools` block). The voice path reaches the same loop via the `voice=True` flag (terse `voice_system` prompt, same tools + Aegis + sessions). Selection quality is gated by `tests/tool_use_eval.py`. See [`zeus/docs/tool-use-spec.md`](zeus/docs/tool-use-spec.md).
