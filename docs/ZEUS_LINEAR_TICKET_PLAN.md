...
**Follow-up LAB-NEW-G** ✅ Root cause found + fixed (July 2026): VRAM contention on the 3080's 10 GB with `OLLAMA_MAX_LOADED_MODELS=2`. The real cause was **not** chat-vs-embedder-vs-reranker (reranker is off in prod). It was that the small-LLM tag (`ZEUS_OLLAMA_SMALL_MODEL=qwen2.5:7b-instruct`) differed from the chat tag (`qwen2.5:7b-instruct-q4_K_M`). The two share a weight digest, but Ollama keys loaded runners by requested **name**, so a single `/chat/message` (chat qwen + fact-extraction small-LLM + retrieval embed) needed **3 distinct ~4.7 GB runners**, evicting one every turn (120s cold reload, retrieval timeouts). Fix: set `ZEUS_OLLAMA_SMALL_MODEL` and the allowed-models list to the exact chat tag so chat + small-LLM share one runner (embedder is #2). Verified: `/chat/message` + `/classify` both 200 in <11s, `api/ps` shows exactly 2 runners. Reranker sidecar for the 5080 path is still LAB-NEW-F. Separate fragility spotted: fastembed's BM25 model caches to ephemeral `/tmp/fastembed_cache`, so a container **recreate** (`up -d`, not `restart`) wipes it and an interrupted re-download leaves a stale `.lock` that hangs hybrid retrieval; worth a persistent cache volume.
