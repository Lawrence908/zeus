...
- **P8 - Observability + streaming (core done).**
  ...
  **P8b (done):** (1) per-node **argonaut transcript** surfacing - `transcript.py` globs `~/.claude/projects/*/<session_id>.jsonl` by the node's `session_id` and returns a compact event view; `GET /swarm/runs/{id}/nodes/{node}/transcript`; the Swarm app shows a per-node `log` toggle. Best-effort: the sandbox worker writes its transcript inside the ephemeral container, so only the host `claude` worker leaves one (the UI says so when absent). (2) **CI poll** - `ci.py` runs `gh pr checks <pr_url>` on demand (stateless, no new column); `GET /swarm/runs/{id}/ci` returns a rollup (passing/failing/pending/none); the app shows a CI badge next to the PR link. 10 new tests.
