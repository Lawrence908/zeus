...
**Aegis / NemoClaw:** Compose file includes a comment block clarifying that the full NemoClaw + OpenShell stack is **not** a Zeus compose service image; it runs on the host (daedalus). Zeus still applies in-process Aegis when enabled. See **`zeus/docs/nemoclaw-ops.md`** for the live install.
